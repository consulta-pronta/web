...
const borderColor = computed(() => {
	...
	switch (props.theme) {
		...
		case "primaryLight":
			return "border-primaryLight"
		case "error":
			return "border-error"
		case "textLight":
			return "border-textLight"
		default:
			return "border-textDark"
	}
})
const backgroundColor = props.mode === "fill" ? props.theme : "transparent"

const dynamicStyle = [
	`bg-${backgroundColor} ${textColor.value}`,
	props.mode === "outline" ? `border-2 ${borderColor.value}` : "",
	props.goto?.length ? "justify-start" : "justify-center",
...
<template>
	<button
		...
		:class="[dynamicStyle, isDisabled ? 'contrast-50' : 'cursor-pointer hover:brightness-90']"
		@click="redirect()"
	>
		<span class="material-symbols-rounded select-none">
			<div v-if="state === 'sync'" class="animate-spin">loop</div>
