...
<template>
	<button @click="openMobile()" type="button" class="flex justify-center items-center size-12 bg-primary rounded-4xl absolute z-50 cursor-pointer shadow-xl/20 m-2 lg:hidden">
		<span v-if="openedMobile" class="material-symbols-rounded text-3xl! text-textLight">close</span>
		<span v-else class="material-symbols-rounded text-3xl! text-textLight">menu</span>
	</button>

	<aside class="h-screen bg-primary flex-col items-center justify-between transition-[width] duration-500 shrink-0 py-4 lg:sticky z-49 shadow-[5px_0_10px_-2px_rgba(0,0,0,0.3)]"
	...
	>
		<section class="w-full flex flex-col overflow-hidden">
			<BaseLogo class="mx-auto mt-10 lg:mt-0 transition-all object-cover duration-600 h-25"
